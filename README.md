...
实验二：五题代码中可视化以及计算函数等主要部分的实现

实验三：优化代码结构，数据的量化，数据文件的导出
...
实验二：优化代码结构，数据的量化，数据文件的导出

实验三：相关函数的编写，PCA和K均值聚类的实现
...
陶希：优化代码结构，数据的量化，数据文件的导出

林明锋：相关函数的编写，PCA和K均值聚类的实现

#### 实验说明：

实验三聚类和数据处理采用c++实现，其中还涉及Eigen库的使用。可视化使用python实现。

#### 项目结构说明：

实验代码为一个源文件和一个show.py在置于 库/实验一/Codes文件夹中
...
**运行的编译平台：**实测聚类和数据处理visual studio 2017可运行，可视化IDLE可运行
...
本实验中该库主要提供PCA中求矩阵特征值和特征向量的函数，相关的函数有pseudoEigenvalueMatrix()和pseudoEigenvectors()

2.Matplotlib

本实验中该库主要提供可视化工具，相关的函数有scatter(),legend(loc='best'),grid(True),show()等

**编写的类：**

为避免代码文件结构冗余，本实验中不使用类。
...
MatrixXf C(50, 50);//使用Eigen库存放要求特征值和特征向量的矩阵

MatrixXf vec, val;//存放特征值和特征向量
...
该函数用来求协方差矩阵的特征值和特征向量并找到特征值最大的两个特征向量组成矩阵，特征向量矩阵与零均值化转置矩阵相乘后转置得到降维后矩阵

#### 关于代码&难题与解决：

1.本实验采用c++语言，因为缺少对矩阵运算的函数，并且PCA数据降维中必须要求矩阵特征值和特征向量，所以必须要使用c++矩阵库Eigen。

2.PCA的具体步骤是将原矩阵转置后零均值化，求协方差矩阵，得到协方差矩阵的特征值和特征向量。找到最大的两个特征值对于的特征向量组成矩阵，与零均值化矩阵相乘得到转置的降维化矩阵，最后进行一次转置即完成对数据的降维。

3.K均值聚类的步骤为遍历一次数据，为每项数据分配类，根据数据所属的类重新计算类中心，若类中心有变化则重复，若类中心均无变化则结束。

#### 总结：

本实验采用c++语言进行多维数据降维和K均值聚类，用python语言进行可视化。尽管有些难点且错误频发，在经过小组间充分讨论和交流后都能将各种问题逐一解决。通过本次实验，我们小组各成员加深了对数据降维和K均值聚类的认识，丰富了团队协作的经验，获益匪浅。
